...
export async function convertEstimateToClientAndInvoice(
  ...
  try {
    ...
    // Use provided dateIssued or default to today's local date
    // We derive local date parts to avoid UTC drift (e.g., 11 PM PST = next day in UTC)
    let dateIssuedParts: ReturnType<typeof parseDateParts>;
    if (invoiceData.dateIssued) {
      dateIssuedParts = parseDateParts(invoiceData.dateIssued);
    } else {
      const today = new Date();
      const todayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;
      dateIssuedParts = parseDateParts(todayStr);
    }
    if (!dateIssuedParts) {
      throw new Error("Invalid dateIssued format (expected YYYY-MM-DD)");
    }

    const dateIssued = toUtcDateFromParts(dateIssuedParts);

    // Calculate due date based on frequency (times per year)
    const frequency = invoiceData.frequency || 2;
    const dateDue =
      calculateDateDueFromParts(dateIssuedParts, frequency) ?? dateIssued;

    const reminderSettings = invoiceData.paymentReminders;
    const paymentReminders =
      ...
      reminderSettings.frequency
        ? {
            ...
            nextReminderDate: calculateNextReminderDateFromParts(
              dateIssuedParts,
              reminderSettings.frequency,
            ),
          }
