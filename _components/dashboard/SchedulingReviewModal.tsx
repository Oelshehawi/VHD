...
export default function SchedulingReviewModal({
  ...
  const requestId = request?._id ? request._id.toString() : undefined;
  const clientId = client?._id ? client._id.toString() : undefined;
  const clientName = (client as any)?.clientName || "Unknown Client";
  ...
  const handleConfirmWithInvoice = async (sourceInvoiceId: string) => {
    ...
    try {
      const result = await confirmSchedulingWithInvoice({
        requestId,
      ...
      if (result.success) {
        toast.success(
          `Invoice ${result.invoiceId} created. Confirmation email sent to client.`,
        );
  ...
  const handleSendAlternatives = async () => {
    ...
    // Convert Date objects to the expected format
    const validAlternatives = alternativeDates
    ...
    try {
      ...
      if (result.success) {
        toast.success("Alternative times sent to client.");
        onClose();
      } else {
        toast.error(result.error || "Failed to send alternatives");
  ...
  const updateAlternativeDate = (index: number, date: Date | undefined) => {
  ...
  return (
    <>
      ...
      {pendingSelection && showConfirmDialog && clientId && requestId && (
        <SchedulingConfirmDialog
          ...
          onConfirm={handleConfirmWithInvoice}
          clientId={clientId}
          client={client}
          confirmedDate={pendingSelection.date}
          confirmedTime={pendingSelection.time}
